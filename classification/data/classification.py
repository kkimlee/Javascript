...
data_stack = Conv2D(filters=16, kernel_size=(16, 1), name="convolution6", padding='valid', activation='relu')(data_stack)
...
data_stack = Dense(40, activation='relu', name="dense3")(data_stack)
data_stack = Dense(10, activation='relu', name="dense4")(data_stack)
...
for i in range(len(test_data)):
    ...
    history = model.fit(x=train_data[i], y=train_label[i], batch_size=128, epochs=500, validation_data=(test_data[i], test_label[i]))
